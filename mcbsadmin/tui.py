...
HELP_TEXT = [
    ...
    "  /install                    install the latest Bedrock build",
...
class PlayerActions:
    """Small modal offering kick/ban for a player, each with an
    optional reason typed into a prompt on the next screen."""

    def __init__(self, name: str) -> None:
        self.name = name
        self.actions: List[str] = [
            "Kick player", "Ban player", "Cancel",
        ]
        self.sel = 0
...
class WorldsModal:
    """World selector: list of worlds/ folders, switch/add/rename/delete.
...
class AllowlistModal:
    """Server-wide allowlist editor: name list, a bottom button bar."""

    def __init__(self, query_fn, enabled: bool = False) -> None:
    ...
    def toggle_label(self) -> str:
        return "Disable allowlist" if self.enabled else "Enable allowlist"
...
class SettingsModal:
    """Modal to edit the server name (MOTD) and the game port."""

    def __init__(self, config: Config, server_dir: str) -> None:
        self.actions: List[str] = [
            "name", "port", "done",
        ]
        self.motd = (config.get("motd") or "").strip() or "MCBSAdmin managed server"
        self.port = config.get("gameport")  # Bedrock default is 19132
        self.edit_buf = ""
        self.sel = 0
        self.editing: Optional[str] = None  # None | "name" | "port"

    def commit_port(self) -> None:
        """Validate the typed port; blank/invalid keeps the current value."""
...
# (config/property key, label, kind) for the world option editors.
# kind: "int" (typed digits, blank = auto), "bool" (cycle true/false),
#       "choice:a,b,c" (cycle), "action" (opens a sub-menu).
WORLD_FIELDS = [
    ...
    ("tick-distance", "tick distance", "int"),
    ("allow-cheats", "allow cheats", "bool"),
    ("online-mode", "online mode", "bool"),
    ("pvp", "pvp", "bool"),
    ("force-gamemode", "force gamemode", "bool"),
    ("default-player-permission-level",
     "default permission", "choice:visitor,member,operator"),
    ("enable-lan-visibility", "LAN visibility", "bool"),
    ("allow-list", "allowlist", "action"),
]
...
class App:
    def __init__(self, stdscr, config: Config) -> None:
        ...
        if self.ext_pid:
            self.log.append(f"[mcbsadmin] Detected running server (PID {self.ext_pid}).")
    ...
    _STATS_INTERVAL = 0.8
    _MESSAGE_MS = 5.0
    _HOTKEYS = frozenset("SXRIWVEHQ")  # all hotkeys are UPPERCASE
    ...
    def run(self) -> None:
        ...
        if self.installed_ver:
            self.log.append(
                f"[mcbsadmin] Minecraft Bedrock {self.installed_ver} installed; "
                f"files in {server_dir}"
            )
        if not os.path.exists(
            os.path.join(self.config.server_dir(), "bedrock_server")
        ):
            self.log.append(
                "[mcbsadmin] No bedrock_server installed. Press 'I' to install "
                "the latest Bedrock build."
            )
        ...
        while True:
            try:
                ...
                self.log.append("[mcbsadmin] internal error (see traceback)")
    ...
    def _handle_modal_key(self, ch: int) -> bool:
        ...
        if isinstance(m, AllowlistModal):
            return self._handle_allowlist_key(m, ch)
        ...
        if isinstance(m, SettingsModal):
            if m.editing is not None:
                if ch == KEY_BACKSPACE:
                    ...
                    c = chr(ch)
                    if m.editing == "port":
                        if c.isdigit() and len(m.edit_buf) < 5:
                            m.edit_buf += c
                    else:
                        m.motd += c
                return True
            if ch == KEY_UP:
                ...
                if m.sel < len(m.actions) - 1:
                    self._settings_start_edit(m)
                else:
                    self._apply_settings(m)
            return True
        return True

    def _settings_start_edit(self, m: SettingsModal) -> None:
        label = m.actions[m.sel]
        if label == "name":
            m.editing = "name"
        elif label == "port":
            m.editing = "port"
            m.edit_buf = "" if m.port is None else str(m.port)
    ...
    def _reason_prompt(self, title, prompt, on_submit) -> None:
        """Open a prompt for a kick/ban reason; Esc returns to the player menu."""
    ...
    def _open_allowlist_modal(self) -> None:
        self.prev_modal = self.modal  # keep the world options underneath
        props = read_properties(
            os.path.join(self.config.server_dir(), "server.properties")
        )
        # the config's world overrides are the source of truth (they're what
        # a start writes into server.properties); fall back to the file
        world = dict(self.config.get("world") or {})
        value = world.get("allow-list") or props.get("allow-list", "false")
        enabled = str(value).lower() in ("true", "1")
        self.modal = AllowlistModal(self._allowlist_query, enabled=enabled)

    def _allowlist_path(self) -> str:
        return os.path.join(self.config.server_dir(), "allowlist.json")

    def _allowlist_query(self) -> List[str]:
        # world options (and thus this editor) are only reachable while the
        # server is stopped, so read the on-disk allowlist.json.
        return read_allowlist_file(self._allowlist_path())

    def _allowlist_add(self, m: AllowlistModal) -> None:
        name = m.buf.strip()
        m.buf = ""
        m.editing = False
        if not name:
            return
        if self._server_running():
            self.server.send_command(f"allowlist add {name}")
        else:
            add_allowlist_entry(self._allowlist_path(), name)
        self._notify(f"{name} added to allowlist.")
        threading.Thread(target=m._load, daemon=True).start()

    def _allowlist_remove(self, m: AllowlistModal, name: str) -> None:
        if self._server_running():
            self.server.send_command(f"allowlist remove {name}")
        else:
            remove_allowlist_entry(self._allowlist_path(), name)
        self._notify(f"{name} removed from allowlist.")
        threading.Thread(target=m._load, daemon=True).start()

    def _allowlist_toggle(self, m: AllowlistModal) -> None:
        """Flip allow-list in server.properties, keeping the entries."""
        value = "true" if not m.enabled else "false"
        set_property(
            os.path.join(self.config.server_dir(), "server.properties"),
            "allow-list",
            value,
        ...
        # write the allowlist back the other way
        world = dict(self.config.get("world") or {})
        world["allow-list"] = value
        self.config.set("world", world)
        self.config.save()
        m.enabled = not m.enabled
        # keep the world-options row behind us in sync so its trailing
        # ">" marker reflects the new state without reopening the menu
        if isinstance(self.prev_modal, FieldModal):
            self.prev_modal.values["allow-list"] = value
        self._notify(
            f"Allowlist {'enabled' if m.enabled else 'disabled'} (entries kept)."
        )

    def _handle_allowlist_key(self, m: AllowlistModal, ch: int) -> bool:
        if m.editing:
            if ch == KEY_BACKSPACE:
                m.buf = m.buf[:-1]
            elif ch in (KEY_ENTER, 13):
                self._allowlist_add(m)
            elif ch in (KEY_ESCAPE,):
                m.editing = False
                m.buf = ""
            elif 32 <= ch < 127:
        ...
        if ch in (KEY_ENTER, 13):
            ...
            if m.sel == n:
                ...
                self._allowlist_toggle(m)
            elif m.sel == n + 2:
                self.modal = self.prev_modal
                self.prev_modal = None
            else:
                self._allowlist_remove(m, m.names[m.sel])
        return True
    ...
    def _field_row_text(self, m: FieldModal, idx: int, mw: int) -> str:
        ...
        if kind == "action":
            # the allowlist editor is active only while allow-list is on
            active = str(m.values.get("allow-list")).lower() in ("true", "1")
    ...
    def _save_world_options(self, values: dict) -> None:
        ...
        for key, _label, kind in WORLD_FIELDS:
            ...
            cfg[key] = str(v)
        cfg.pop("level-name", None)  # the Worlds menu owns the active world
    ...
    def _worlds_dir(self) -> str:
        return os.path.join(self.config.server_dir(), "worlds")
    ...
    def _current_world(self) -> str:
        ...
        return str(self.config.get("level") or props.get("level-name") or "level")
    ...
    def _world_rename(self, m: WorldsModal, old: str, raw: str) -> None:
        ...
        if os.path.exists(dst):
            ...
            return
        os.replace(src, dst)
        # keep the world's display name in sync with the new folder name
        try:
            with open(os.path.join(dst, "levelname.txt"), "w",
    ...
    def _world_delete(self, m: WorldsModal, name: str) -> None:
        ...
        if self._current_world() == name:
            self._set_level(remaining[0] if remaining else "level")
    ...
    def _settings_row(self, m: SettingsModal, i: int, w: int):
        ...
        label = m.actions[i]
        if label == "name":
            text = prefix + "name: " + truncate(m.motd, w - 11)
            if m.editing == "name" and i == m.sel:
                text += "_"
            return text, "settings:name"
        if label == "port":
            if m.editing == "port" and i == m.sel:
                value = truncate(m.edit_buf + "_", w - 13)
            else:
                value = "auto" if m.port is None else str(m.port)
            return prefix + "port: " + value, "settings:port"
    ...
    def _dispatch_click(self, token: str) -> bool:
        ...
        if token == "start":
            ...
            m = self.modal
            if isinstance(m, SettingsModal):
                if action in ("name", "port"):
                    m.sel = m.actions.index(action)
                    self._settings_start_edit(m)
                elif action == "done":
                    if m.editing is not None:
                        if m.editing == "port":
        ...
        elif token.startswith("allowlist-remove:"):
            name = token.split(":", 1)[1]
            m = self.modal
            if isinstance(m, AllowlistModal):
                self._allowlist_remove(m, name)
        elif token == "allowlist-add":
            m = self.modal
            if isinstance(m, AllowlistModal):
                m.editing = True
                m.buf = ""
        elif token == "allowlist-toggle":
            m = self.modal
            if isinstance(m, AllowlistModal):
                self._allowlist_toggle(m)
        return False
    ...
    def _quick_install(self) -> None:
        ...
        self.install_progress = (0, 0)
        self._notify("Installing the latest Bedrock build…")

        self.log.append("[mcbsadmin] Downloading the latest Bedrock build…")
    ...
    def _on_install_done(self, ok: bool, version: Optional[str], result: str) -> None:
        ...
        if ok:
            ...
            self.last_result = f"Installed Bedrock {version}."
            self.log.append(f"[mcbsadmin] Installed Bedrock {version}. Press 'S' to start.")
        else:
            self.last_result = f"Install failed: {result}"
            self.log.append(f"[mcbsadmin] Error: {result}")
    ...
    def _apply_settings(self, m: SettingsModal) -> None:
        self.config.set(
            "motd", (m.motd.strip() if m.motd.strip() else "MCBSAdmin managed server")
        )
        if m.port:
            self.config.set("gameport", m.port)
        try:
            self.server.setup_files()
        except Exception:  # noqa: BLE001
            pass
        self.modal = None
        self._notify("Server settings saved (applied on start).")
    ...
    def _modal_geom(m, h: int, w: int):
        ...
        elif isinstance(m, AllowlistModal):
    ...
    def _record_hits(self) -> None:
        ...
        if m is not None:
            ...
            self._modal_hits(m, _mh, _mw, my, mx)
    ...
    def _draw_header(self) -> None:
        ...
        try:
            ...
            left_w = max(0, w - len(status_text) - 2)
            ...
            # the status + pid sit right-aligned in the header box
            x = max(0, w - len(status_text))
    ...
    def _stats_rows(self) -> List[tuple]:
        ...
        if pid:
            ...
            # live RSS of the server process vs the system's total RAM, so
            # the bar shows how much of the box the server alone is using
            sm = self.srv_mem
            if sm is not None and total:
                rows.append(("Server RAM",
                             f"{fmt_bytes(sm)} / {total / 1024.0 ** 3:.1f} GiB",
                             sm / total))
            else:
                rows.append(("Server RAM", fmt_bytes(sm) if sm else "…", None))
        sc = self.sys_cpu
    ...
    def _game_port(self) -> int:
        ...
        try:
            return int(cfg.get("gameport") or 19132) if cfg else 19132
        except (TypeError, ValueError):
            return 19132
    ...
    def _draw_stats(self) -> None:
        ...
        #   19132  Uptime   <- port on the left, "Uptime" label on the right
        ...
        for label, value, frac in self._stats_rows():
            ...
            if frac is not None and avail >= 3:
                # three-line form: bar on top, value under it, label underneath, e.g.:
                #   [######----------]
                #   900 MiB / 4.0 GiB
                #   Server RAM
                ...
                self._winline(win, y + 1, 1, self._centered(value, w),
                              theme.attr("stats"))
                self._winline(win, y + 2, 1, self._centered(label, w),
                              theme.attr("dim"))
                y += 3
    ...
    def _draw_input(self) -> None:
        ...
        try:
            ...
            start = 0
            if len(vis) > maxtext:
                # left-anchored window that follows the cursor: the text is
                # read left to right and the caret never leaves the screen
                start = max(0, min(self.input_pos - maxtext + 1, len(vis) - maxtext))
                vis = vis[start:start + maxtext]
            self.stdscr.addstr(y, plen, vis, theme.attr("input"))
            self.stdscr.move(y, clamp(plen + (self.input_pos - start), plen, w - 1))
            self.stdscr.noutrefresh()
        except curses.error:
            pass

    def _modal_title(self, m, w: int) -> str:
        """Modal border title, one per modal type."""
        if isinstance(m, PlayerActions):
            return f" PLAYER: {m.name} "
        if isinstance(m, ConfirmModal):
            return m.title
        if isinstance(m, WorldsModal):
            return " WORLDS "
    ...
    def _modal_hits(self, m, mh: int, mw: int, my: int, mx: int) -> None:
        """Populate the click hitmap for the current modal (mirrors drawing)."""
        if isinstance(m, PlayerActions):
            tokens = [
                "kick:" + m.name,
                "ban:" + m.name,
            ...
            for i, label in enumerate(m.actions):
                self._hit(my + 1 + i, mx + 1, len(truncate(label, mw - 4)),
                          tokens[i])
        elif isinstance(m, SettingsModal):
            ...
            for i in range(rows):
                if i >= len(names):
                    break
                self._hit(my + 1 + i, mx + 1, len(truncate(names[i], mw - 4)),
                          "allowlist-remove:" + names[i])
            bar = mh - 3
            x = 1
            tokens = ["allowlist-add", "allowlist-toggle", "close"]
            for i, label in enumerate(m.actions()[len(names):]):
                btext = "[" + label + "]"
            ...
            for i in range(shown):
                ...
                if idx >= len(m.worlds):
                    break
                self._hit(my + y, mx + 1, mw - 2, "worlds:select:" + m.worlds[idx])
                y += 3
            bar = my + (mh - 2)
            x = mx + 1
            ...
            for i, label in enumerate(m.actions):
                self._hit(my + 3 + i, mx + 1, len(truncate(label, mw - 4)),
                          "confirm:" + label.lower())
    ...
    def _draw_modal(self) -> None:
        ...
        if m is None:
            return
        mh, mw, my, mx = self._modal_geom(m, h, w)
        ...
        try:
            ...
            pass
        self._winline(win, 0, 2, truncate(self._modal_title(m, w), mw - 4),
                      theme.attr("status_ok", curses.A_BOLD))
        sel = getattr(m, "sel", 0)
        if isinstance(m, PlayerActions):
            for i, label in enumerate(m.actions):
                attr = theme.attr("input_accent" if i == sel else "input",
                                  curses.A_BOLD if i == sel else 0)
                text = (" > " if i == sel else "   ") + truncate(label, mw - 4)
                self._winline(win, 1 + i, 1, text, attr)
        elif isinstance(m, SettingsModal):
            for i, _label in enumerate(m.actions):
                attr = theme.attr("input_accent" if i == sel else "input",
                                  curses.A_BOLD if i == sel else 0)
                text, _token = self._settings_row(m, i, mw)
                self._winline(win, 1 + i, 1, truncate(text, mw - 4), attr)
        elif isinstance(m, AllowlistModal):
            names = m.names
            ...
            for i, label in enumerate(buttons):
                ...
                self._winline(win, bar, x, btext, theme.attr("input_accent"))
                x += len(btext) + 1
            if m.loading:
                self._winline(win, 1, 1, "Fetching allowlist…", theme.attr("dim"))
            elif m.error:
            ...
            for i in range(rows):
                ...
                if idx >= len(m.fields):
                    break
                attr = theme.attr("input_accent" if idx == sel else "input",
                                  curses.A_BOLD if idx == sel else 0)
                text = self._field_row_text(m, idx, mw)
                self._winline(win, 1 + i, 1, text, attr)
        ...
        self._modal_hits(m, mh, mw, my, mx)
        win.noutrefresh()
        self._modal_win = win
